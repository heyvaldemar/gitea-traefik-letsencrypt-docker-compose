#!/usr/bin/env bash
# gitea-restore-database.sh [backup-file-name]
#
# Replaces the Gitea database with one of the dumps the backups service wrote.
#
#   ./gitea-restore-database.sh               list and ask
#   ./gitea-restore-database.sh <file-name>   restore that one
#
# EVERY PATH, NAME AND CREDENTIAL COMES FROM THE RUNNING BACKUPS CONTAINER.
# The previous version carried giteadb and giteadbuser as literals and its own
# copy of the backup directory, so any of the three set differently in .env
# pointed it at a database or directory the stack does not use.
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than gitea.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-gitea}"
APP_SERVICE="gitea"
...
DB_NAME="$(env_of GITEA_DB_NAME)"; DB_USER="$(env_of GITEA_DB_USER)"
...
if ! docker exec "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
  ...
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using Gitea." >&2
  exit 1
fi
